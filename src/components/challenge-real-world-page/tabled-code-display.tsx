...
import { Button } from "@/components/challenge-real-world-page/ui/button"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/challenge-real-world-page/ui/tabs"
...
export function TabbedCodeDisplay({
  examples,
  lightTheme,
  darkTheme,
    ...
    if (highlighted) {
      return (
        <div
          ...
          className={cn(
            "h-full w-full overflow-auto font-mono text-xs bg-[#24292f]",
    ...
    <div className="w-full">
      <Tabs defaultValue={examples[0]?.id} className="w-full">
        <div className="flex items-center justify-between mb-4">
          <TabsList className={`grid w-full grid-cols-${columnCount} flex bg-slate-300 border border-d`}>
            {examples.map((example) => (
              <TabsTrigger key={example.id} value={example.id} className="text-sm font-h3">
                {example.title}
              </TabsTrigger>
            ))}
          </TabsList>
